...
`git checkout e16c8af`：表示回到'e16c8af'这个版本。但是会出现'detachde HEAD'，你问我具体为啥？[往后看！](#headmastermain的区别)  
或者，用'~'后加数字表示"前几个"。如：  
`git checkout HEAD~2`：回到前俩个版本  
- 值得注意的是，'HEAD'和'master（main）'是有区别的。关于'HEAD'和'master（main）'是什么，有什么区别，我们在分支时会[详细介绍](#headmastermain的区别)
...
[HEAD/master（main）的区别](#headmastermain的区别)
...
- 需要注意的是，这其实是使用了“快传模式”。因为'master（main）'是'dog'分支的“爹”，也就是'dog'是从'master（main）'分出去的。
...
- 如果发生冲突，Git会告诉你哪个文件出问题了。如果是文本文件的话，直接改完文本文件具体内容后，先add再commit就行。如果不是文本文件，如图片什么的，使用'--ours'或者'--theirs'来选择用谁的。如，有俩个master（main）的“子分支”，我们要将'cat'合并到'dog'分支。其中apple.jpg有冲突.  
...
### HEAD/master（main）的区别
ok，到这里，大家应该都对分支有了一个简单的认识。我这里再向大家区分一下'HEAD'和'master（main）'的区别。
- 'master（main）'是默认分支的名字。是可以改名的，改成阿猫阿狗都可以。指向一个commit。  
-  自46 岁的非裔美国人 George Floyd 在街头光天化日之下被白人警察按倒在街头7分钟导致死亡这件事发酵后，自 2.28 版本起，Git 开始支持对默认分支名'main'进行配置以取代一直以来的'master'用法。    
...
- Git Folw
  ...
  1. Master（main） 分支：主干分支，也就是完整上线版本1.0，2.0之类的。切记不要commit到这个分支上
  ...
  4. Release 分支：上线前测试。当Develop 分支测试足够成熟时，开出一个 Release 分支来，然后做发布前的准备工作。这个分支对应的是预发环境。
     - 如果 Release 分支上的代码测试合格，那么需要把 Release 分支向 Master（main） 分支和Develop 分支同时合并，以保证代码的一致性。然后再把 Release 分支删除掉。  
  5. Hotfix 分支：是用于处理生产线上代码的紧急发生的bug，每个线上代码的紧急 bug 都需要开一个 Hotfix 分支，完成后，向 Develop 分支和 Master（main） 分支上合并。合并完成后，删除 Hotfix 分支。
- GitHub Flow  
...
git push
```  
- 前面提到的操作指令中的`git push -u origin master`，其中，-u（-upstream）是把origin设置成为了master（main）的远端节点，他会指向并跟踪master（main）。通常远端节点是远端的分支，但是设置本地也可以。  
如果没说清楚远端节点则每次push就该说明将哪个交给哪个分支：  
`git push origin master（main）`  
否则，只执行git push的话，git就会因为你没说清而不执行
- `git push origin master（main）:dog`  
如果在远端不想要master（main）这个名字，而想要dog分支，它会起到作用的。  
...
本文是作者个人整理，难免有些地方理解不正确或者不到位。欢迎各位指正（邮箱：A2356360027@163.com）  
